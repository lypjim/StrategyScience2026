#!/usr/bin/env python3
"""
Conservative PDF Anonymizer
- Reads metadata (authors, emails, institutions)
- Downloads PDF from Dropbox
- Redacts ONLY specific terms (not entire sections)
...
OUTPUT_DIR = "anonymized_conservative"
...
def redact_pdf_conservative(input_path: str, output_path: str, terms: Set[str]):
    """Conservative redaction: remove only specific terms, preserve structure."""
    try:
        import fitz

        doc = fitz.open(input_path)

        # Only check first page for author info
        page = doc[0]

        # Redact specific terms found in the metadata
        redacted_count = 0
        for term in terms:
...
def main():
    ...
    response_id = str(sample_row.get("ResponseId", ""))
    filename = str(sample_row.get("Paper_Name", ""))
    terms = get_sensitive_terms(sample_row)

    print(f"\n🧪 Testing: {filename[:50]}...")
    print(f"   Terms to redact: {len(terms)}")
    ...
    final_path = os.path.join(OUTPUT_DIR, f"CONSERVATIVE_{response_id}.pdf")
    ...
    try:
        ...
        success, redacted_count = redact_pdf_conservative(local_path, final_path, terms)

        if success:
            print(f"   ✓ Redacted {redacted_count} terms")
            print(f"   ✓ Saved to: CONSERVATIVE_{response_id}.pdf")
        else:
