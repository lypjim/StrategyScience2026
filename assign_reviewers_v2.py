...
def load_reviewers_from_firebase() -> Dict[str, Reviewer]:
    ...
    try:
        ...
        for rid, rdata in data.items():
            ...
            print(
                f"   ✓ {reviewer.name}: {reviewer.method} | Max: {reviewer.max_papers or '∞'} | Keywords: {len(reviewer.keywords)}"
            )
...
def get_available_reviewers(
    paper: Paper, reviewers: Dict[str, Reviewer], current_load: Dict[str, int]
) -> List[str]:
...
def rank_reviewers_for_paper(
    ...
    for rid in available_reviewers:
        ...
        # Load penalty - prefer less loaded reviewers while respecting hard caps
        load = current_load.get(rid, 0)
        capacity = (
            reviewer.max_papers if reviewer.max_papers > 0 else 20
        )  # Use realistic max for fairness
        # Lower penalty to prioritize reviewer expertise over just load distribution
        load_penalty = (load / capacity) * 0.2  # Reduced from 0.3 to 0.2
...
def load_papers(csv_path: str) -> List[Paper]:
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            # Clean title by removing newlines and extra whitespace
